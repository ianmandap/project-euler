...
# Let E(n) be the expected value of F(P) over all permutations P
  ...
  while array != sorted
    (0...(max_index - 1)).each do |i|
      if array[i] > array[i + 1]
        array.insert(0, array.delete(array[i + 1]))
        iterations += 1
        break
      end
    end
...
def main
  ...
  arr = (1..4).to_a.permutation.to_a
  values = []
  arr.each { |a| values << first_sort(a) }
  p expected_value(values).round(2)
...
main
# n of 10 has 10! permutations or 3_628_800;
# n of 30 has 30! permutations
# Needs more analysis
